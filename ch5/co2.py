...
def kernel(x,y,params):
    k1 = params[0]**2*torch.exp(-(x-y)**2/(2*params[1]**2))
    k2 = params[2]**2*torch.exp(-(x-y)**2/(2*params[3]**2)-2*torch.sin(np.pi*(x-y))**2/(params[4]**2))
    k3 = params[5]**2*(1+(x-y)**2/(2*params[7]*params[6]**2))**(-params[7])
    k4 = params[8]**2*torch.exp(-(x-y)**2/(2*params[9]**2))+params[10]**2*(x==y)
    return k1+k2+k3+k4

def gram_matrix(xs,params):
    return torch.tensor([[kernel(x1,x1,params) for x1 in xs] for x2 in xs])

def surprisal(xs,ys,params):
    cov = gram_matrix(xs,params)
    return 0.5 * (ys.T @ torch.linalg.torch.solve(ys.view(-1,1),cov) + torch.log(torch.linalg.det(cov)))
...
if __name__=="__main__":
    ...
    params = torch.tensor(np.random.normal(size=11),requires_grad=True)
    optimizer = torch.optim.Adam([params])
    print("Optimising")
    while True:
        optimizer.zero_grad()
        loss = surprisal(torch.tensor(x_train),torch.tensor(y_train),params)
        print(loss)
        loss.backward()
        optimizer.step()
    """
